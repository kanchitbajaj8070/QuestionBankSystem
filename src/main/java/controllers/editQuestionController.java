...
import model.questionsAllDetials;
...
//not implemented since it is set to disabled 
    ...
    public void handleSaveButtonAction(MouseEvent event) {
        ...
        else if (correct_answer != null && editQuestionType.getValue().equalsIgnoreCase("objective") && editQuestionCorrectChoice.getValue().equalsIgnoreCase("no option")) {//case of subjective question original and objective afterwards
        ...
        try {
            Boolean ans = handler.getInstance().editQuestionDatabaseChangesHandler(question_text, category_name, subcategory_name, handler.getInstance().viewImage(editQid), type, difficulty, marks, loginController.loggedUser, firstchoice, secondchoice, thirdchoice, fourthchoice, correct_answer, time_assigned, editQid);
            System.out.println("inserted question" + "	closing edit screenn!!!!!!!!!!!");
            if (ans == true) {
    ...
    public void initialize(URL arg0, ResourceBundle arg1) {
		...
		questionsModel obj = (questionsModel) c.getTable().getSelectionModel().getSelectedItem();
        editQid = obj.getId();
